...
class ProtLigTokenizer:
    ...
    def tokenize_protein(self, protein, prot_coords=None):
        ...
        if isinstance(prot_coords, np.ndarray):
            ...
            tokens.extend([self.prot_coords_start, self.prot_coords_end])
            numbers.extend((len(filtered_atoms) + 4) * [1])
            return tokens, numbers
        else:
    ...
    def tokenize_ligand(self, ligand_SMILES, ligand_atoms=None, lig_coords=None):
        ...
        if isinstance(lig_coords, np.ndarray):
            ...
            tokens.extend([self.lig_coords_start, self.lig_coords_end])
            numbers.extend((len(tokens_smi) + 4) * [1])
            return tokens, numbers
        else:
    ...
    def untokenize(self, tokens, numbers=None):
        ...
        current_type = None
        prot_backbone_tokens = {'N', 'CA', 'C', 'O'}
        coord_tokens = {'[x]', '[y]', '[z]'}
        special_tokens = {self.prot_start, self.prot_end, self.lig_start, self.lig_end,
                         self.prot_coords_start, self.prot_coords_end, 
                         self.lig_coords_start, self.lig_coords_end}
        for i, token in enumerate(tokens):
            if token == self.prot_start:
                ...
                result[current_type] = ''
                if self.lig_end not in tokens[i:]:
                    lig_tokens = []
                    skip_initial_coords = True
                    for j in range(i+1, min(i+200, len(tokens))):
                        t = tokens[j]
                        if t == self.prot_end:
                            continue
                        if skip_initial_coords and t in coord_tokens:
                            continue
                        skip_initial_coords = False
                        if t in {self.prot_coords_start, self.lig_coords_start, 
                                 self.prot_coords_end, self.lig_coords_end}:
                            break
                        if not skip_initial_coords and t in coord_tokens:
                            break
                        if t not in prot_backbone_tokens:
                            lig_tokens.append(t)
                    result['LigSmiles'] = ''.join(lig_tokens)
                    return result
                ...
                if current_type:
                    if current_type == 'LigSmiles':
                        if token not in coord_tokens and token != self.prot_end and token not in prot_backbone_tokens:
                            result[current_type] += token
                    elif current_type == 'Prot':
                        result[current_type].append(token)
                    elif current_type in ['ProtCoords','LigCoords'] and token in coord_tokens:
