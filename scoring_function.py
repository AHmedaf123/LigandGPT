...
from GNNdataset import ProteinPocketFeaturizer
VDW_RADII_MAP = {
...
def convert_receptor_to_pocket_features(
    receptor_file: str,
    _cache: Dict[str, Optional[torch.Tensor]] = {}
) -> Optional[torch.Tensor]:
    """Extract protein pocket features from a receptor PDB/PDBQT file.
    
    Implements [B3]: y_bio = f(P, L) by providing the protein (P) representation
    to the GNN bioactivity predictor.
    
    Args:
        receptor_file: path to the receptor PDB or PDBQT file
        _cache: mutable default dict used as a simple in-memory cache
    
    Returns:
        torch.Tensor of shape [num_pocket_atoms, 40] or None
    """
    if receptor_file in _cache:
        return _cache[receptor_file]
    
    # Try PDB file first, then PDBQT
    pdb_path = receptor_file
    if receptor_file.endswith('.pdbqt'):
        pdb_path = receptor_file.replace('.pdbqt', '.pdb')
    
    prot_features = None
    for path in [pdb_path, receptor_file]:
        if os.path.exists(path):
            prot_features = ProteinPocketFeaturizer.featurize_pocket(path)
            if prot_features is not None:
                break
    
    # Cache the result (limit cache size)
    if len(_cache) < 200:
        _cache[receptor_file] = prot_features
...
def predict_bioactivity_for_batch(
    ...
    gnn_model: Any,
    protein_pocket_features: Optional[torch.Tensor] = None
) -> List[Optional[float]]:
    """Predict bioactivity scores for a batch of ligands given a protein pocket.
    
    Implements [B3]: y_bio = f(P, L) — the GNN receives both protein pocket
    features and ligand graph features for prediction.
    
    Args:
        smiles_list: list of SMILES strings for ligands
        gnn_model: the trained AdvancedGNN model
        protein_pocket_features: optional tensor [num_prot_atoms, 40] for the
            target protein pocket. If None, falls back to ligand-only prediction.
    """
    ...
    try:
        ...
        # [B3] Attach protein pocket features to each ligand graph
        if protein_pocket_features is not None:
            prot_x_list = []
            prot_batch_list = []
            for i, graph in enumerate(valid_graphs):
                prot_x_list.append(protein_pocket_features)
                prot_batch_list.append(
                    torch.full((protein_pocket_features.size(0),), i, dtype=torch.long)
                )
        
        batch_graph = Batch.from_data_list(list(valid_graphs))
        
        # Attach batched protein features to the batch object
        if protein_pocket_features is not None and prot_x_list:
            batch_graph.prot_x = torch.cat(prot_x_list, dim=0)
            batch_graph.prot_batch = torch.cat(prot_batch_list, dim=0)
            # Move to same device as model
            device = next(gnn_model.parameters()).device
            batch_graph.prot_x = batch_graph.prot_x.to(device)
            batch_graph.prot_batch = batch_graph.prot_batch.to(device)
...
def score_single_molecule(
    ...
    # [B3] Pass protein pocket features for y_bio = f(P, L)
    bioactivity_predictions = predict_bioactivity_for_batch(
        [smiles], gnn_model, protein_pocket_features=protein_pocket_features
    )
...
def calculate_scores_for_batch(
    ...
    for smiles in smiles_batch:
        ...
        score_result, current_molecule_cache = score_single_molecule(
            ...
            protein_pocket_features=protein_pocket_features
        )        
...
def load_gnn_model_from_checkpoint(model_path: str) -> Optional[Any]:
    ...
    try:
        ...
        # strict=False allows loading old checkpoints without protein encoder weights
        # (protein encoder layers will be randomly initialized and need fine-tuning)
...
_GNN_MODEL = None
_TEMP_DIR = None
_PROTEIN_POCKET_CACHE = {}  # Cache: receptor_file -> pocket features tensor
...
def calculate_scores(
    ...
    """Calculate all scoring metrics for a batch of SMILES.
    
    Implements [B3]: y_bio = f(P, L) by extracting protein pocket features
    from the receptor file and passing them to the GNN bioactivity predictor.
    """
    oracles, gnn_model, temp_dir = initialize_scoring_system()
    
    # [B3] Extract protein pocket features for y_bio = f(P, L)
    protein_pocket_features = convert_receptor_to_pocket_features(receptor_file)
