...
def get_curriculum_weights(curriculum_stage: int) -> Dict[str, float]:
    # Rotational curriculum: 75% dominant group, 12.5% each for the other two
    # Groups: Affinity (dock,bio) | Chemical (qed,sa) | Physical (energy,steric,torsion)
    if curriculum_stage == 1:
        # Stage 1: Affinity focus (75%)
        return {
            'docking': 0.375,
            'bioactivity': 0.375,
            'qed': 0.065,
        ...
        # Stage 2: Chemical viability focus (75%)
        return {
            'docking': 0.065,
            'bioactivity': 0.060,
            'qed': 0.400,
        ...
        # Stage 3: Physical stability focus (75%)
        return {
            'docking': 0.065,
            'bioactivity': 0.060,
            'qed': 0.065,
...
class MultiObjectiveOptimizer:
    def __init__(self, batch_size: int = 32, archive_size: int = 100):
        ...
        logger.info("3-Stage Rotational Curriculum: Affinity Focus (0-200) → Chemical Focus (200-400) → Physics Focus (400+)")    
    def update(
        self,
    ...
    def update_curriculum_stage(self, current_step):
        ...
        if current_step < 200:
            self.current_stage = 1  
            stage_name = "Affinity Focus (75%)"
        elif current_step < 400:
            self.current_stage = 2 
            stage_name = "Chemical Viability Focus (75%)"
        else:
            self.current_stage = 3  
            stage_name = "Physical Stability Focus (75%)"        
        if old_stage != self.current_stage:
            ...
            if self.current_stage == 2:
                logger.info(f"Stage 2: Chemical focus — QED (0.40) + SA (0.35), Affinity reduced to 12.5%")
            elif self.current_stage == 3:
                logger.info(f"Stage 3: Physics focus — Energy (0.30) + Steric (0.25) + Torsion (0.20), others at 12.5%")
            logger.info(f"{'='*60}\n")
            self.reward_baseline = 0.0      
    def _compute_single_reward(self, scores: Dict[str, float], smiles: Optional[str] = None) -> float:
        ...
        if self.current_stage == 1:
            # Stage 1: Affinity focus (75%) — reward dominated by docking + bioactivity
            is_valid = any(
                optional_scores.get(prop) is not None 
                for prop in ['docking', 'bioactivity']
            )            
            if not is_valid:
                return -1.0
            reward = 0.0
            ...
            if docking is not None:
                docking_normalized = (max(-12, min(-4, docking)) + 12) / 8.0
                reward += docking_normalized * 0.375            
            if bioactivity is not None:
                bio_normalized = (max(-12, min(-4, bioactivity)) + 12) / 8.0
                reward += bio_normalized * 0.375            
            # Light regularisers (12.5% each group)
            ...
            if sa is not None:
                ...
                reward += sa_normalized * 0.060
            energy = optional_scores.get('energy')
            if energy is not None and energy < 200.0:
                reward += (1.0 - energy / 200.0) * 0.050
            steric = optional_scores.get('steric')
            if steric is not None and steric < 15.0:
                reward += (1.0 - steric / 15.0) * 0.045
            torsion = optional_scores.get('torsion')
            if torsion is not None and torsion < 5.0:
                reward += (1.0 - torsion / 5.0) * 0.030
            return float(np.clip(reward, -1.0, 1.0))
        elif self.current_stage == 2:
            # Stage 2: Chemical viability focus (75%) — reward dominated by QED + SA
            is_valid = any(
                optional_scores.get(prop) is not None 
                for prop in ['qed', 'sa']
            )            
            ...
            if sa is not None:
                sa_normalized = 1.0 - ((sa - 1.0) / 9.0)
                reward += sa_normalized * 0.350            
            # Light regularisers (12.5% each group)
            docking = optional_scores.get('docking')
            if docking is not None:
                docking_normalized = (max(-12, min(-4, docking)) + 12) / 8.0
                reward += docking_normalized * 0.065
            bioactivity = optional_scores.get('bioactivity')
            if bioactivity is not None:
                bio_normalized = (max(-12, min(-4, bioactivity)) + 12) / 8.0
                reward += bio_normalized * 0.060
            energy = optional_scores.get('energy')
            if energy is not None and energy < 200.0:
                reward += (1.0 - energy / 200.0) * 0.050
            steric = optional_scores.get('steric')
            ...
            # Stage 3: Physical stability focus (75%) — reward dominated by energy + steric + torsion
            is_valid = any(
                optional_scores.get(prop) is not None 
                for prop in ['energy', 'steric', 'torsion']
            )            
            if not is_valid:
                return -1.0
            reward = 0.0
            ...
            if energy is not None:
                reward += max(0.0, 1.0 - energy / 200.0) * 0.300
            steric = optional_scores.get('steric')
            if steric is not None:
                reward += max(0.0, 1.0 - steric / 15.0) * 0.250
            torsion = optional_scores.get('torsion')
            if torsion is not None:
                reward += max(0.0, 1.0 - torsion / 5.0) * 0.200            
            # Light regularisers (12.5% each group)
            docking = optional_scores.get('docking')
            if docking is not None:
                docking_normalized = (max(-12, min(-4, docking)) + 12) / 8.0
